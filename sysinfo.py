#!/usr/bin/env python3
"""
sysinfo.py – Raspberry Pi Systemübersicht
Keine externen Abhängigkeiten, nur Python-Stdlib + psutil (optional)
"""
...
print(f"{B}{C}")
print("╔══════════════════════════════════════════════════╗")
print("║           Raspberry Pi – Systemübersicht         ║")
print(f"╚══════════════════════════════════════════════════╝{RST}")
...
if freq_path.exists():
    ...
    print(f"  {B}Taktfrequenz:{RST} {freq_mhz} MHz  ({freq_ghz:.2f} GHz)")
else:
    freq = cmd("vcgencmd measure_clock arm 2>/dev/null | awk -F= '{printf \"%.0f\", $2/1000000}'")
    print(f"  {B}Taktfrequenz:{RST} {freq or 'n/a'} MHz")

# CPU-Auslastung über /proc/stat (kein psutil nötig)
...
cpu_use = cpu_percent()
print(f"  {B}Auslastung:{RST}   {bar(cpu_use)}  ({color(cpu_use, 60, 85, '{:.1f}')}%)")

cores = os.cpu_count()
print(f"  {B}Kerne:{RST}        {cores}")
...
print(f"  {B}Nutzung:{RST}      {bar(ram_pct)}")
print(f"  {B}Gesamt:{RST}       {total_mb} MB")
print(f"  {B}Genutzt:{RST}      {color(ram_pct, 70, 90, '{:.0f}')} MB  ({used_mb} MB)")
print(f"  {B}Verfügbar:{RST}    {avail_mb} MB")
print(f"  {B}Puffer/Cache:{RST} {buffers_mb + cached_mb} MB")
...
# ── Verbundene Nutzer ─────────────────────────────────────────────────────────
section("Verbundene Nutzer")

who_out = cmd("who").splitlines()
if who_out:
    print(f"  {B}Aktive Sessions:{RST} {len(who_out)}")
    for line in who_out:
        parts = line.split()
        ...
        host_info = f"von {host}" if host else ""
        print(f"  • {G}{user}{RST} @ {tty}  {date} {ttime}  {host_info}")
else:
    print("  Keine aktiven Nutzer (nur diese Session).")
...
for line in cmd("ip -br addr show").splitlines():
    ...
    addrs = " ".join(parts[2:]) if len(parts) > 2 else "keine IP"
